 ...
 */
import { Axios, AUTH_CRED, Config, TOP_TOPICS, DAILY_DEVIATIONS, DEVIATION, SEARCH_USER, USER_GALLERY, } from "./Config.js";
/**
 * Get deviations top topics
 * @returns {Promise<Object>}
 */
export const topTopics = async () => {
    let result;
 ...
 */
export const dailyDeviations = async () => {
    let result;
    const Auth = await AUTH_CRED();
    if (Auth.status && Auth.access_token) {
 ...
 * @returns {Promise<Object>}
 */
export const deviationId = async (userid) => {
    let result;
    const Auth = await AUTH_CRED();
    if (Auth.status && Auth.access_token) {
 ...
 * @return {Promise<Object>}
 */
export const userGallery = async (username) => {
    let result;
    const Auth = await AUTH_CRED();
    if (Auth.status && Auth.access_token) {
